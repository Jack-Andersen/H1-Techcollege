    ...
    **/
    class Program
    {
        ...
        */
        ...
        public static bool GemTrans(Konto konto, string text, float beløb) {
            var saldo = findSaldo(konto);
            if (saldo + beløb < 0) return false;
            var transaktion = new Transaktion();
            transaktion.text = text;
            transaktion.amount = beløb;
            transaktion.balance = transaktion.amount + saldo;
            transaktion.date = DateTime.Now;
        ...
        public static float findSaldo(Konto k) {
            ...
            foreach(var t in k.transaktioner) {
                if (t.date > senesteDato) {
                    senesteDato = t.date;
                    seneste = t;
                }
            }
            return seneste.balance;
        }
